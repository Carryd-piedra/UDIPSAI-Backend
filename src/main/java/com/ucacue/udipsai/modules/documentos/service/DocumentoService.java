...
public class DocumentoService {
    ...
    public Documento saveDocumentoDos(MultipartFile file, Long pacienteId, String nombre) throws IOException {
        ...
        // Crear DocumentoAdjunto y actualizar JSON del paciente
        DocumentoAdjunto docAdjunto = new DocumentoAdjunto(
                savedDoc.getId(),
                nombre, // ← aquí usamos el nombre ingresado por el usuario
                filePath.toString()
        );
        paciente.getDocumentos_paciente().add(docAdjunto);
    ...
    public void deleteDocumento(Long id) {
        ...
        try {
            if (archivo.exists() && !archivo.delete()) {
                System.err.println("No se pudo eliminar el archivo físico: " + documento.getUrl());
            }
        } catch (Exception e) {
        ...
        if (paciente == null) {
            throw new RuntimeException("El documento no tiene paciente asociado");
        }

        // 3. Modificar lista documentos_paciente directamente
        List<DocumentoAdjunto> documentosPaciente = paciente.getDocumentos_paciente();

        boolean eliminado = documentosPaciente.removeIf(doc -> doc.getId() == id);
        if (!eliminado) {
            System.err.println("Documento no encontrado en documentos_paciente del paciente.");
        } else {
            paciente.setDocumentos_paciente(documentosPaciente); // opcional si el setter es necesario
            pacienteRepositorio.save(paciente);
        }

        // 4. Eliminar el documento de la tabla
