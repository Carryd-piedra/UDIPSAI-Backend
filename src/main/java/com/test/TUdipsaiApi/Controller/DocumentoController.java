...
//    }
    ...
    public ResponseEntity<Documento> createDocumento(
        ...
        try {
            Documento savedDocumento = documentoService.saveDocumentoDos(file, pacienteId, nombre);
            return ResponseEntity.ok(savedDocumento);
        } catch (IOException e) {
            return ResponseEntity.status(500).build();
        }
    ...
    public ResponseEntity<byte[]> descargarDocumento(@PathVariable Long id) {
        try {
            ...
            HttpHeaders headers = new HttpHeaders();
            headers.set(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=documento_" + id + ".pdf");
